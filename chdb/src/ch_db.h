...
public:
    ...
    //map key to transaction, show which tx hold the key lock
    std::map<int,int> keyToTx;
    int insertKeyToTx(int key,int tx_id){
        auto it = this->keyToTx.find(key);
        if(it == this->keyToTx.end()){
            this->keyToTx.insert(
                std::pair<int,int>(key,tx_id)
            );
        }
        else{
            it->second = tx_id;
    ...
    int removeKeyToTx(int key){
        auto it = this->keyToTx.find(key);
        if(it != this->keyToTx.end()){
            this->keyToTx.erase(key);
        }
    }
    int tryLock(int key,int tx_id);
    int releaseLock(int key,int tx_id);
private:
